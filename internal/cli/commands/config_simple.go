...
// NewSimpleConfigCommand creates the config command for simple CLI
func NewSimpleConfigCommand() *simplecli.Command {
...
Examples:
  ...
  azemailsender-cli config env > .env`,
				Run: runSimpleConfigEnv,
			},
		},
	}
}

func runSimpleConfigInit(ctx *simplecli.Context) error {
...
func runSimpleConfigShow(ctx *simplecli.Context) error {
	debug := ctx.GetBool("debug")
	quiet := ctx.GetBool("quiet")
	jsonOutput := ctx.GetBool("json")
...
func runSimpleConfigEnv(ctx *simplecli.Context) error {
	debug := ctx.GetBool("debug")
	quiet := ctx.GetBool("quiet")
	jsonOutput := ctx.GetBool("json")
